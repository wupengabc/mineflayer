...
const injectAnyEvent = require('./any_event')
const plugins = {
  ...
  packets26_1: require('./plugins/packets26_1')
}
...
function createBot (options = {}) {
  ...
  // PW's "地标" menus are click-only plugin GUIs despite using a generic
  // container type. Callers can replace this list or set window.serverManaged.
  options.serverManagedWindows = options.serverManagedWindows ?? ['地标']

  // Proxy support: tunnel the TCP connection through SOCKS5 / HTTP / HTTPS.
  // We install our own options.connect before delegating to mc.createClient,
  // which causes NMP's default tcp_dns to skip net.connect entirely. The
  // resolved host/port are populated by NMP's tcp_dns (defaults applied)
  // before this connect callback fires, so we can safely read them here.
  if (options.proxy) {
    const { createProxyConnector } = require('./proxy')
    const connector = createProxyConnector(options.proxy)
    options.connect = (client) => {
      try {
        const host = options.host || 'localhost'
        const port = options.port || 25565
        const socket = connector(host, port)
  ...
  // Patched before any listener is attached so onAny sees even the earliest
  // events ('connect', 'error', ...) that fire before plugins inject.
  injectAnyEvent(bot)
  ...
  // Deferred connection: expose bot.connect() to open the socket on demand.
  // Registry init (next) must wait until after connect, because the client
  // version is only known once the socket is up / version negotiation ran.
  if (!autoConnect && typeof bot._client.startConnection === 'function') {
    bot.connect = () => bot._client.startConnection()
  }
  if (!autoConnect) {
    // Version is unknown until the connection starts; wait for connect_allowed
    // (auto-version path) or connect (fixed-version path) before init.
    if (options.version === false) bot._client.once('connect_allowed', next)
    else bot._client.once('connect', next)
  } else if (!bot._client.wait_connect) next()
