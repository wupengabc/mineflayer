...
function inject (bot) {
  ...
  function getChestType (chestBlock) { // Returns 'single', 'right' or 'left'
    if (bot.supportFeature('doesntHaveChestType')) {
      ...
      if (!facing || !FACING_MAP[facing]) return 'single'
  ...
  bot._client.on('block_action', (packet) => {
    ...
    if (blockName === 'noteblock') { // Pre 1.13
      ...
      if (blockName === 'chest' || blockName === 'trapped_chest') {
        ...
        if (chestType === 'right') {
          const facing = parseChestMetadata(block).facing
          const facingMap = FACING_MAP[facing]
          if (facingMap) {
            const index = Object.values(facingMap).indexOf('left')
            const cardinalBlock2 = Object.keys(facingMap)[index]
            const block2Position = block.position.plus(CARDINALS[cardinalBlock2])
            block2 = bot.blockAt(block2Position)
          }
          // Unknown facing (e.g. block state IDs we don't decode on 26.1+):
          // fall through with block2 = null. The chestLidMove event still
          // fires for the right half; consumers that want the paired block
