...
function inject (bot, options) {
  ...
  function spawn () {
    const sendsPlayerLoadedOnChunk = bot.supportFeature('playerLoaded')
    const sendsPlayerLoadedOnSpawn = bot.supportFeature('sendsPlayerLoadedPacket') || bot.protocolVersion >= 769
    if (sendsPlayerLoadedOnSpawn && !sendsPlayerLoadedOnChunk) bot._client.write('player_loaded', {})
    bot.emit('spawn')
  }
