...
function inject (bot) {
  ...
  function teamHandler (packet) {
    ...
    // 26.1+ encodes the team options as a bitflags object. Older protocol
    // schemas exposed the same value as the flat `friendlyFire` property.
    const friendlyFire = packet.friendlyFire ?? packet.flags?.friendly_fire
