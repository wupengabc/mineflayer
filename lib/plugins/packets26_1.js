...
// on internal NMP paths. Tags protocol-stage errors with the negotiated version
...
//
// Activated only for Minecraft 26.1.x and 26.2 (protocols 775 and 776);
...
function inject (bot) {
  // Feature gate: only active on 26.1.x/26.2. bot.protocolVersion is
  // populated in lib/loader.js's `next()` before `inject_allowed` is emitted,
  // so this check is safe at inject time.
  if (bot.protocolVersion !== 775 && bot.protocolVersion !== 776) return

  let clientLoaded = false
  const pendingAttacks = []

  const resetClientLoaded = () => {
    clientLoaded = false
    pendingAttacks.length = 0
  }
  bot._client.on('login', resetClientLoaded)
  bot._client.on('respawn', resetClientLoaded)
  bot._client.on('start_configuration', resetClientLoaded)
  bot.on('playerLoaded', () => {
    clientLoaded = true
    while (pendingAttacks.length) {
      const attack = pendingAttacks.shift()
      attack()
    }
  })

  bot._queueAttackUntilLoaded = (attack) => {
    if (clientLoaded) {
      attack()
    } else {
  ...
  bot._client.on('low_disk_space_warning', () => {
    if (typeof bot._warn === 'function') {
      bot._warn(`low_disk_space_warning: server reports low disk space (protocolVersion=${bot.protocolVersion})`)
    } else {
      console.warn(`[mineflayer] low_disk_space_warning: server reports low disk space (protocolVersion=${bot.protocolVersion})`)
    }
  })
  ...
  // when the rawPacket originated from this bot's 26.1/26.2 protocol path.
  //
  // The event payload follows the spec exactly:
  //   { packetId, state, protocolVersion: bot.protocolVersion,
  //     name: 'unimplemented_<state>_0x<id>' }   // <id> is lowercase hex
  // ---------------------------------------------------------------------------
  bot._client.on('rawPacket', (raw) => {
    if (!raw || raw.protocolVersion !== bot.protocolVersion) return
    const { state, packetId } = raw
  ...
  bot._client.prependListener('position', (packet) => {
    try {
      ...
      bot.emit('error', tagWith261(err, bot.protocolVersion))
    }
  })
  ...
  if (bot.supportFeature('playerLoaded')) {
    ...
    bot.on('chunkColumnLoad', sendPlayerLoaded)
  }
}
