...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  // ---------------------------------------------------------------------------
  // Task 11.2 helpers — client_tick_end cadence.
  //
  // Servers on protocol >= 768 (1.21.2+) expect exactly one serverbound
  // tick_end (a.k.a. `client_tick_end`) per physics tick; the packet exists in
  // the toServer table from 1.21.2 onward. The protocol.json registers this
  // packet under the legacy NMP name `tick_end`, so we write that name.
  //
  // The minecraft-data feature is named `sendsClientTickEndPacket`
  // (1.21.2 → latest). Gating on that feature keeps the version mapping in
  // minecraft-data as the single source of truth and automatically covers
  // 26.1/26.1.2/26.2 (protocols 775/776).
  //
  // IMPORTANT: a previous revision looked up the non-existent feature name
  // `clientTickEnd`, which always resolved to false — the bot never sent
  // tick_end on 26.1/26.2 and got kicked by anti-cheats that validate the
  // vanilla per-tick packet cadence.
  // ---------------------------------------------------------------------------
  const sendsClientTickEnd = bot.supportFeature('sendsClientTickEndPacket')
  const sendsPlayerInput = bot.supportFeature('newPlayerInputPacket')
  ...
  // Monotonic physics-tick counter. Replaces wall-clock time for the
  // keep-alive cadence so the send schedule is defined in the same unit the
  // server anti-cheat uses (20Hz ticks), not in jittered wall-clock ms.
  let tickCounter = 0
  ...
  const lastSent = {
    ...
    pitch: 0,
    onGround: false,
    tick: 0, // tickCounter value at the time of the last position keep-alive
  ...
  function doPhysics () {
    ...
    // Recover from timer jitter (Windows setInterval has a ~15.6ms granularity,
    // so a 50ms interval actually fires every ~57–64ms and would otherwise run
    // the tick loop at ~17.5Hz instead of 20Hz). Catching up the missed ticks
    // keeps the tick cadence — and therefore every packet whose validity the
    // server anti-cheat ties to the 20Hz tick boundary (player_input,
    // tick_end, the per-second position keep-alive) — aligned with vanilla.
    //
    // Measured worst case on Windows is a 63–64ms frame, which catches up
    // exactly one tick; the PHYSICS_CATCHUP_TICKS cap only guards against a
    // pathological stall (breakpoint, GC pause), never normal jitter.
    let catchupTicks = 0
  ...
  function tickPhysics (now) {
    ...
    if (sendsPlayerInput && shouldUsePhysics) {
  ...
  function sendPacketPosition (position, onGround) {
    ...
    lastSent.flags = { onGround, hasHorizontalCollision: bot.entity.isCollidedHorizontally } // 1.21.3+
  ...
  function updatePosition (now) {
    ...
    const positionUpdated = lastSent.x !== position.x || lastSent.y !== position.y || lastSent.z !== position.z ||
      ...
      // Vanilla measures "1 second" as 20 ticks, not wall-clock time: a vanilla
      // client idling at a locked 20fps sends its keep-alive move exactly every
      // 20th tick. Counting physics ticks (instead of rounding wall-clock ms to
      // the nearest 50ms) keeps the keep-alive aligned to the same 20Hz tick
      // boundary the server anti-cheat uses, so an idle bot does not drift a
      // fraction of a tick per second away from the vanilla cadence.
      (tickCounter - lastSent.tick) >= 20
    ...
    if (positionUpdated && lookUpdated) {
      ...
      lastSent.tick = tickCounter // only reset if positionUpdated is true
      ...
      bot._client.write('flying', {
        ...
        flags: { onGround: bot.entity.onGround, hasHorizontalCollision: bot.entity.isCollidedHorizontally } // 1.21.3+
      })
    }
  ...
  bot._client.on('position', (packet) => {
    ...
    // Any new forced move supersedes a pending delayed echo (see below).
    ...
    if (bot.supportFeature('teleportUsesOwnPacket')) {
      // 26.3 (protocol 777) extended ServerboundAcceptTeleportationPacket to
      // carry the position and rotation being acknowledged, so the server can
      // validate the correction instead of trusting the id alone.
    ...
    if (respawnTimer > 0 && Date.now() - respawnTimer < 2000) {
      ...
      // While physics is suspended for the delay, residual velocity from the
      // pre-death state must not leak into the UI or the next simulation.
      stopResidualMotion()
      ...
      setTimeout(() => {
        // A newer forced move (or death / config switch / disconnect) already
        // echoed a position: flushing this stale one would move the bot
        // backwards in time and trip movement anti-cheats.
  ...
  let respawnTimer = 0
  // Generation counter for forced moves (clientbound position). Every forced
  // teleport invalidates a previously scheduled delayed position_look echo so
  // a stale echo can never be flushed after a newer one (server-side
  // anticheats see that as the position moving backwards in time).
  let forcedMoveGeneration = 0
  const stopResidualMotion = () => {
    if (bot.entity?.velocity) bot.entity.velocity.set(0, 0, 0)
  }
  bot.on('mount', () => {
