...
module.exports.isServerManagedWindow = isServerManagedWindow

function isServerManagedWindow (window, matchers) {
  if (!window || !Array.isArray(matchers)) return false
  const title = window.title?.toString?.() ?? ''
  return matchers.some(matcher => {
    if (typeof matcher === 'string') return title.includes(matcher)
    if (matcher instanceof RegExp) return matcher.test(title)
    return false
  })
}
...
function inject (bot, { hideErrors, serverManagedWindows = [] }) {
  ...
  async function activateBlock (block, direction, cursorPos, forceLook) {
  ...
  async function clickWindow (slot, mouseButton, mode) {
    ...
    let changedSlots
    const isServerManagedMenuClick = bot.supportFeature('stateIdUsed') &&
      slot >= 0 && slot < window.inventoryStart &&
      (window.serverManaged === true || isServerManagedWindow(window, serverManagedWindows))
    if (isServerManagedMenuClick) {
      // Plugin menus cancel vanilla item movement. In 1.17.1+ the client must
      // report no predicted slot/cursor changes, otherwise the server rejects
      // the click as an invalid operation.
      changedSlots = []
    } else if (bot.supportFeature('transactionPacketExists')) {
  ...
  bot._client.on('set_slot', (packet) => {
    ...
    if (!window || window.id !== packet.windowId) {
      // A trailing update for the player-inventory section of a window that
      // was closed client-side still describes bot.inventory.
