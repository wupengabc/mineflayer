...
function inject (bot) {
  ...
  function addNewNonPlayer (entityId, uuid, entityType, pos) {
    ...
    updateEntityPos(entity, pos)
    if (typeof pos.objectData === 'number') entity.objectData = pos.objectData
    entity.uuid = uuid
    return entity
  }
  ...
  bot._client.on('spawn_entity', (packet) => {
    ...
    if (packet.velocity) {
      entity.velocity.update(bot.protocolVersion >= 775
        ? packet.velocity
        : conv.fromNotchVelocity(new Vec3(packet.velocity.x, packet.velocity.y, packet.velocity.z)))
    }
    bot.emit('entitySpawn', entity)
  })
  ...
  bot._client.on('entity_destroy', (packet) => {
    ...
    packet.entityIds.forEach((id) => {
      ...
      // Skip entities that were never tracked (e.g. spawned out of render
      // distance before we joined). Creating a ghost via fetchEntity just to
      // immediately delete it wastes an allocation and fires a spurious
      // entityGone event for something we never knew about.
      if (!entity) return
  ...
  bot.useOn = useOn
  bot.interactEntity = interactEntity
  bot.moveVehicle = moveVehicle

  function swingArm (arm = 'right', showHand = true, priority = false) {
    // 26.3 deleted ServerboundSwingPacket and added ServerboundPunchPacket,
    // which is an EMPTY record: the hand is no longer on the wire and the
    // server derives the swing animation from the held item
    // (see ServerGamePacketListenerImpl.handlePunch). Sending the 26.1/26.2
    // `arm_animation` payload would be an unknown packet id on 26.3.
    if (bot.protocolVersion >= 777) {
      writePacket('punch', {}, priority)
      return
    }
    const hand = arm === 'right' ? 0 : 1
    const packet = {}
    if (showHand) packet.hand = hand
    writePacket('arm_animation', packet, priority)
  }

  function useOn (target, offhand = false, position) {
    // TODO: check if not crouching will make make this action always use the item
    return interactEntity(target, {
      hand: offhand ? 1 : 0,
      position
    })
  }

  function interactEntity (target, options = {}) {
    const hand = options.hand === 1 || options.offhand === true ? 1 : 0
    ...
    return useEntity(target, 0, undefined, undefined, undefined, options.priority === true, hand)
  }

  function attack (target, swing = true) {
    const sendAttack = () => {
      attackPackets(target, swing)
    }

    if (bot._queueAttackUntilLoaded) {
      bot._queueAttackUntilLoaded(sendAttack)
    } else {
      sendAttack()
    }
  }

  function attackPackets (target, swing) {
    // arm animation comes before the use_entity packet on 1.8
    if (bot.supportFeature('armAnimationBeforeUse')) {
      if (swing) {
        swingArm('right', true, true)
      }
      useEntity(target, 1, undefined, undefined, undefined, true)
    } else {
      useEntity(target, 1, undefined, undefined, undefined, true)
      if (swing) {
        swingArm('right', true, true)
  ...
  function useEntity (target, leftClick, x, y, z, priority = false, hand = 0) {
    const sneaking = bot.getControlState('sneak')

    // 26.1+ split the legacy use_entity (action 0/1/2) into two distinct packets:
    //   action 1 (attack) -> serverbound `attack` carrying just the entity id
    //   action 0/2 (interact / interact_at) -> serverbound `use_entity` with no
    //     mouse field; the optional aim coordinates and the hand are dedicated
    //     fields, and the full Vec3 is encoded as an LpVec3.
    if (bot.protocolVersion >= 775) {
      if (leftClick === 1) {
        writePacket('attack', { entityId: target.id }, priority)
        return
      }
      const location = (x !== undefined && y !== undefined && z !== undefined)
        ? { x, y, z }
        : { x: 0, y: 0, z: 0 }
      writePacket('use_entity', {
        target: target.id,
        hand,
        location,
        sneaking
  ...
  function writePacket (name, params, priority) {
    const writer = priority && bot._client.writePriority
      ? bot._client.writePriority
      : bot._client.write
    writer.call(bot._client, name, params)
  }
